...
export const AchievementCard: React.FC<CardProps> = ({ entry, onOpenFolder, onMore }) => {
  ...
  return (
    <div className="bg-[#121418]/90 border-[3px] border-[#ffd700]/80 rounded-xl p-5 hover:border-[#ffd700] transition-all duration-300 achievement-card-glow shadow-[0_0_20px_rgba(255,215,0,0.25)] group relative overflow-hidden aspect-[4/3] max-w-[512px] w-full flex flex-col justify-between mx-auto md:mx-0">
      ...
      <div className="flex-1 flex flex-col min-h-0 relative z-10">
        ...
        {entry.imgPath ? (
          <div className="w-full flex-1 min-h-0 rounded overflow-hidden mb-3 border border-amber-500/20 dark:border-slate-800 bg-slate-900/50 flex items-center justify-center">
            <img 
              src={entry.imgPath} 
              alt={entry.title} 
      ...
      <div className="flex gap-2 text-xs relative z-10 pt-2 border-t dark:border-slate-800/50 border-slate-100 shrink-0">
        <button 
          ...
          className="flex items-center gap-1 px-2.5 py-1.5 dark:bg-slate-800 bg-slate-100 hover:bg-slate-200 dark:hover:bg-slate-700 dark:text-slate-200 text-slate-700 rounded border dark:border-transparent border-amber-200 transition-colors"
        >
          <FileText size={12} />
          <span>More</span>
        </button>
