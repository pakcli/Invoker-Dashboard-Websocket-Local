...
export const CertCard: React.FC<CardProps> = ({ entry, onOpenFolder, onMore, thinnerCard, isChecked = false, onToggleChecked }) => {
  const [color1, color2, color3] = getOrbColors(entry.skill);

  return (
    <div 
      onClick={() => onMore(entry)}
      className={`bg-[#12161b]/95 border border-slate-800 rounded-xl hover:border-slate-200/60 transition-all duration-300 shadow-md group relative overflow-hidden max-w-[512px] w-full flex flex-col justify-between mx-auto md:mx-0 cursor-pointer ${
